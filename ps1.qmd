...
commuterstable <- data.frame(
  ...
  N = c(length(commuters$AGE), length(commuters$female),length(commuters$educ_lhs), 
  ...
  SD = c(sd(commuters$AGE), sd(commuters$female),sd(commuters$educ_lhs), 
         ...
         sd(commuters$educ_college), sd(commuters$educ_adv), sd(commuters$TRANTIME), 
         sd(commuters$INCTOT) / 1e6),
...
knitr::kable(commutersrounded, 
             caption = "Table 2: Summary Statistics for 2024 ACS Commuters Sample",
             digits = 2)
```
